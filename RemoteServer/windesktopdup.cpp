...
Error WinDesktopDup::Initialize() {
    ...
    dxgiAdapter = nullptr;
    if (FAILED(hr)) {
        return "dxgiAdapter->EnumOutputs failed: %v";
...
Error WinDesktopDup::CaptureNext() {
    if (!DeskDupl)
        return "DeskDupl false";
    ...
    if(frameInfo.PointerPosition.Position.x > 0 && frameInfo.PointerPosition.Position.y > 0)
...
//    }
    ...
    if(frameInfo.PointerPosition.Visible == true ){
        uint32_t* bufPtr = (uint32_t*)Latest.Buf.data();
        const uint8_t* mousePtr = mouse_map;
        for (int y=cursorPos.y-1; y<cursorPos.y-1+30; y++) {
            for (int x=cursorPos.x-1; x<cursorPos.x-1+20; x++) {
                if( *mousePtr != 0xff &&y>=0 && y<Latest.Height
                                     && x>=0 && x<Latest.Width ){
                    bufPtr[y*Latest.Width+x] = 0x00000000;
                }
                mousePtr++;
            }
        }
    ...
    if(ok){
        return "CreateTexture2D Map ok";
    }else{
        return "CreateTexture2D Map fail";
    }
}
